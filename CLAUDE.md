...
## 10. Agent Package (253 Tools)

The agent package is a single JSON file any LLM can consume, containing 253 tools with input schemas and endpoint mappings. It is the load-bearing piece of the **Claude Max front door**: drop the JSON into a Claude conversation and Claude can transact on the user's behalf without further hand-holding.
...
| `tools` | 253 entries. Each has `name`, `description`, `input_schema` (JSON Schema), and `endpoint` (`{method, path}`). |
