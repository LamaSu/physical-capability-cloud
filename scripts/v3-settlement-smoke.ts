...
async function main() {
  ...
  async function fundedMilestone(escrow: Address, jobId: string, stepIdStr: string) {
    ...
    await payerWallet.writeContract({
      address: escrow, abi: MilestoneEscrowV3ABI, functionName: "addMilestone",
      args: [stepIdBytes, payer.address, amountUnits, 0n, 0n, 0, jobId],
    }).then((h) => pub.waitForTransactionReceipt({ hash: h }));
    await payerWallet.writeContract({
      address: cfg.token!, abi: ERC20_ABI, functionName: "approve", args: [escrow, amountUnits],
    }).then((h) => pub.waitForTransactionReceipt({ hash: h }));
    await payerWallet.writeContract({ address: escrow, abi: MilestoneEscrowV3ABI, functionName: "fund", args: [] })
      .then((h) => pub.waitForTransactionReceipt({ hash: h }));
    await payerWallet.writeContract({
      address: escrow, abi: MilestoneEscrowV3ABI, functionName: "submitEvidence", args: [0n, evidenceHash],
    }).then((h) => pub.waitForTransactionReceipt({ hash: h }));
  ...
  // ── 2. Mode B: oracle-attested, fee FROM the attestation ────────────────────
  {
    ...
    await payerWallet.writeContract({ address: escrow, abi: MilestoneEscrowV3ABI, functionName: "submitAttestation", args: [0n, uid] })
      .then((h) => pub.waitForTransactionReceipt({ hash: h }));
    await payerWallet.writeContract({ address: escrow, abi: MilestoneEscrowV3ABI, functionName: "release", args: [0n] })
      .then((h) => pub.waitForTransactionReceipt({ hash: h }));
