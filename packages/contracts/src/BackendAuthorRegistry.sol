...
/// @dev Name suffixed `_BAR` to avoid clashing with the same-named interface
///      declared in ContributorNFT.sol (Solidity disallows duplicate
///      file-level identifiers in the same compilation unit).
interface IRateScheduleRegistry_BAR {
    function exists(bytes32 scheduleHash) external view returns (bool);
}

/**
 * @title  BackendAuthorRegistry
 * @notice Generic on-chain authorship registry for vendor backend modules
 *         executed through PCC adapters. Each vendor (PyLabRobot, Hamilton
 *         Venus, Trilobio, future) deploys their OWN instance of this
 *         contract with their own namespace ("pylabrobot:", "hamilton:",
 *         "trilobio:") and modulePath pattern.
 *
 *         Each entry maps a UTF-8 vendor module path (e.g.
 *         "pylabrobot.liquid_handling.backends.hamilton.STAR" or
 *         "venus.devices.STARlet") to a BackendRecord describing the
 *         backing ContributorNFT, the published rate schedule (sha256 hash),
 *         an optional delegated agent id, the off-chain manifest CID, and
 ...
 *         + ai/research/pr-55-generic-refactor-spec-2026-05-26.md):
 *
 *         - Vendor-agnostic via constructor `_namespace` parameter
 ...
 *         - On-chain BackendRegistryDirectory (Phase 2 — env-var directory
 *           handled at the aggregator gate layer)
 */
contract BackendAuthorRegistry {
    // ── Types ────────────────────────────────────────────────────────────

    /**
     * @notice On-chain state for one registered vendor backend.
     *
     ...
     *         attestation flow are documented in docs/AUTHORS.md for the
     *         vendor bridge repo. Address(0) disables recovery entirely.
     */
    address public immutable governanceMultisig;

    // ── Storage-bound namespace + pattern ───────────────────────────────

    /// @notice Vendor namespace prefix used for deterministic ipId derivation.
    ///         Set at construction, immutable thereafter. Examples: "pylabrobot:",
    ///         "hamilton:", "trilobio:", "opentrons:".
    /// @dev    Solidity 0.8.x does not support `immutable` strings, so we use
    ///         a plain storage slot and rely on the constructor + no-setter
    ///         convention for immutability.
    string public namespace;

    /// @notice Free-form regex hint describing the expected modulePath shape for
    ///         this instance. Stored on-chain so block-explorer introspection can
    ///         display the expected pattern without out-of-band documentation.
    ///         The contract does NOT enforce it.
    string public modulePathPattern;
     ...
     *
     *         The namespace is NOT included in the event payload — it's
     *         recoverable by reading registry.namespace() once on indexer
     *         bootstrap, so emitting it on every event would waste gas.
     */
     ...
     * @param _namespace            Vendor namespace prefix (e.g.
     *                              "pylabrobot:", "hamilton:"). MUST end in
     *                              ":" by convention so deriveIpId
     *                              concatenation is well-formed. The
     *                              contract does NOT enforce the trailing
     *                              colon (so vendors can supply alternate
     *                              shapes like "v2:hamilton:" if needed),
     *                              but the deployment script MUST validate.
     *                              Must be 1..32 bytes.
     * @param _modulePathPattern    Free-form regex hint describing the
     *                              expected modulePath shape. Stored
     *                              verbatim and exposed via getter for
     *                              off-chain validation. NOT enforced
     *                              on-chain.
     */
    constructor(
        ...
        string memory _modulePathPattern
    ) {
        require(_contributorNFT != address(0), "Zero ContributorNFT");
        require(_scheduleRegistry != address(0), "Zero ScheduleRegistry");
        require(bytes(_namespace).length > 0, "Empty namespace");
        require(bytes(_namespace).length <= 32, "Namespace too long");
        // Soft check: namespace SHOULD end in ':' so deriveIpId concatenation is well-formed.
        // We don't enforce on-chain (would prevent legitimate namespaces like "v2:hamilton:"),
        // but the deployment script MUST validate.
        contributorNFT = _contributorNFT;
        scheduleRegistry = _scheduleRegistry;
        governanceMultisig = _governanceMultisig;
        namespace = _namespace;
        modulePathPattern = _modulePathPattern;
    }

    // ── Registration ────────────────────────────────────────────────────

    /**
     * @notice Claim attribution for a vendor module path.
     ...
     *         resolves to a real backend in the vendor's namespace via the
     *         vendor-specific verification flow — this contract has no
     *         oracle and trusts the caller for that claim. CID tampering
     *         is benign because the on-chain author identity is derived
     *         from ContributorNFT.ownerOf(), not from the off-chain manifest.
     *
     *         modulePath shape (e.g., enforcing it starts with
    ...
    function register(
        ...
        if (!IRateScheduleRegistry_BAR(scheduleRegistry).exists(scheduleHash)) {
        ...
        external
    {
        ...
        if (!IRateScheduleRegistry_BAR(scheduleRegistry).exists(newScheduleHash)) {
            revert ScheduleNotRegistered();
        }
     ...
     *         flow are documented in the vendor bridge repo; this contract
     ...
     *         a job to the vendor sidecar. Unregistered backends pass through
    ...
    /**
     * @notice Compute the deterministic ipId for a vendor backend per the
     *         validated-answer Q10 rule, parameterized on the per-vendor
     *         namespace:
     *           ipId = keccak256(abi.encodePacked(namespace, modulePath, ":", majorVersion))
     *
     *         Reads the namespace from storage (so this is `view`, not
     *         `pure`). Authors (or the dashboard onboarding flow) call this
     *         once before minting the ContributorNFT to stamp `ipId`
     *         consistently across on-chain + off-chain references.
     */
