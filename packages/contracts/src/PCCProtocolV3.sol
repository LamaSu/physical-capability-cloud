...
import {MilestoneEscrowV3} from "./MilestoneEscrowV3.sol";
import {IPCCOracle} from "./interfaces/IPCCOracle.sol";
import {Clones} from "./libraries/Clones.sol";

/**
 * @title PCCProtocolV3
 * @notice Root protocol factory for the EAS-gated, fee-from-attestation PCC escrows (V3).
 *
 * Companion factory for `MilestoneEscrowV3` (merged in #139). It is a faithful mirror of
 * `PCCProtocolV2` — same immutable fee recipient, same fee bounds + governance machinery,
 * the same `isProtocolEscrow` / `collectFee` accounting, and the same EIP-1167
 * minimal-proxy CLONE pattern — but its factory method `createEscrowV3` clones a shared
 * `MilestoneEscrowV3` implementation (not V2) and `initialize`s the clone with its
 * per-escrow config.
 *
 * WHY THIS EXISTS: `MilestoneEscrowV3` shipped as a DRAFT in #139 WITHOUT a factory, so it
 * was not deployable. `PCCProtocolV2.createEscrowV2` clones a fixed `escrowImplementation`
 * bound at construction (a V2 implementation); that immutable cannot be swapped to a V3
 * implementation. V3 is therefore ADDITIVE: a NEW factory deployed alongside, leaving the
 * live `PCCProtocolV2` and every existing V2 escrow untouched.
 *
 * FAULT ISOLATION (the reason for the clone pattern): every escrow is its own address with
 * its own storage and its own USDC balance. A bug or exploit that drains or bricks escrow A
 * delegatecalls into the shared logic but operates ONLY on A's storage/balance — it
 * physically cannot reach escrow B's funds. The shared EAS wiring (EAS address +
 * `pcc.evidence.v2` schema UID + authorized oracle) lives in the implementation's code as
 * immutables and is read identically by every clone; the per-escrow config
 * (payer/arbiter/token/cwmId/protocolRoot) is written to each clone's own storage by
 * `initialize`.
 *
 * V3 FEE MODEL (the behavioral delta from V2): `MilestoneEscrowV3` reads `feeBps` /
 * `feeRecipient` from the EAS attestation payload (the `pcc.evidence.v2` schema), not from
 * this factory. The factory's `protocolFeeBps` / `feeRecipient` are RETAINED for parity and
 * off-chain consumers (e.g. `calculateFee`) but are NO LONGER consulted by the child's
 * release-time fee math — exactly as V2 RETAINS `oracleVerifier` for V1 parity without it
 * gating the EAS release path. The `collectFee(token, fee)` accounting hook IS still invoked
 * by V3 children for cumulative protocol-wide fee tracking, so this surface is preserved.
 *
 * GAS: a clone is ~45 bytes of runtime behind ~55 bytes of init code, so `createEscrowV3`
 * costs tens-of-thousands of gas. The `new MilestoneEscrowV3(...)` initcode lives in the
 * factory's CREATION code only, so the factory's own runtime stays well under the
 * 24,576-byte EIP-170 limit.
 *
 * Fee bounds (identical to V1/V2):
 *   - Minimum: 10 bps (0.1%) — fee can NEVER be set to zero
 *   - Maximum: 500 bps (5%)
 *   - Default: 235 bps (2.35%)
 *
 * The constructor arg list is intentionally UNCHANGED from `PCCProtocolV2` so the deploy
 * script (`DeployProtocolV3.s.sol`) and ABIs are a clean mirror of the V2 ones. The schema
 * UID threaded to children is the `pcc.evidence.v2` schema UID (a different value from V2's
 * `pcc.evidence.v1` UID — the schema strings differ).
 */
contract PCCProtocolV3 {
    // ── Constants ────────────────────────────────────────────────────
    ...
    /// @dev RETAINED for parity + off-chain consumers. V3 children read the fee recipient
    ///      from the EAS attestation, not from here.
    address public immutable feeRecipient;

    /// @notice The PCC Verification Oracle verifier contract. IMMUTABLE.
    /// @dev Retained for V1/V2 parity (collectFeeWithAttestation). The EAS release gate lives
    ///      in the child escrow's submitAttestation; this is not consulted there.
    ...
    /// @notice The `pcc.evidence.v2` schema UID threaded to every child escrow.
    /// @dev Different value from V2's `pcc.evidence.v1` UID — the V2 schema appends
    ///      `feeBps` + `feeRecipient`, so the registry-derived UID differs. Stored on the
    ///      child as `PCC_EVIDENCE_V2_SCHEMA_UID`.
    ...
    /// @notice The shared MilestoneEscrowV3 implementation that every escrow clones.
    ...
    ///      (eas / pccEvidenceSchemaUid / easOracle). `createEscrowV3` deploys EIP-1167
    ...
    /// @dev RETAINED for parity. NOT consulted by V3 release fee math (fee is attested).
     ...
     * @param _oracleVerifier      IMMUTABLE oracle verifier contract address (V1/V2 parity).
     * @param _eas                 The EAS contract threaded to children (Base + Base Sepolia: 0x42...0021).
     * @param _pccEvidenceSchemaUid The `pcc.evidence.v2` schema UID threaded to children.
     * @param _easOracle           The authorized oracle signer (EAS attester) threaded to children.
     *
     * @dev The constructor deploys ONE shared, LOCKED MilestoneEscrowV3 implementation with
     *      the same EAS wiring it advertises (eas / schemaUid / oracle). Every escrow from
     *      `createEscrowV3` is an EIP-1167 clone of that implementation. Deploying the
     *      implementation here (rather than accepting it as a parameter) guarantees the
     *      clones' immutable EAS wiring can never drift from the factory's. The arg list is
     *      intentionally IDENTICAL to `PCCProtocolV2` so the deploy script/ABI mirror cleanly;
     *      the implementation is an internal deployment detail. The `new MilestoneEscrowV3(...)`
     *      initcode lives in the factory's CREATION code only, so it does NOT count against the
     *      factory's EIP-170 runtime-size limit.
     */
    constructor(
        ...
        // MilestoneEscrowV3's own constructor re-asserts this on the implementation below.
        ...
        escrowImplementation = address(
            new MilestoneEscrowV3(_eas, _pccEvidenceSchemaUid, _easOracle)
        );
    }
    ...
    /**
     * @notice Deploy a new MilestoneEscrowV3 as an EIP-1167 clone of the shared
     ...
     * @return escrow The address of the newly deployed MilestoneEscrowV3 clone.
     */
    function createEscrowV3(
        address payer,
        address arbiter,
        address token,
        bytes32 cwmId
        ...
        // this factory, so release() routes the collectFee accounting hook here). The shared
        // EAS wiring (eas / schemaUid / oracle) is read from the implementation code via
        // delegatecall.
        MilestoneEscrowV3(escrow).initialize(payer, arbiter, token, cwmId, address(this));
     ...
     *         moment `createEscrowV3` is called).
     * @dev Pure off-chain helper for callers that want the escrow address before the tx
     *      lands. Uses the same salt derivation as `createEscrowV3`.
     ...
     * @dev The escrow transfers the fee directly to the attested fee recipient. This
     *      function is called after the transfer for cumulative accounting purposes. Only
     *      callable by factory-deployed escrows. V3 children invoke this via the
     *      `IPCCProtocolV2` interface (the selector + ABI are identical), so no V3-specific
     ...
     * @dev RETAINED for parity. NOTE: V3 release fee math reads from the attestation, so
     *      changing this value does NOT change what V3 escrows charge.
    ...
    // ── Oracle-Gated Settlement (V1/V2 parity) ─────────────────────
     ...
     * @dev Retained for V1/V2 parity. The EAS release gate lives in the child escrow.
     ...
     * @dev RETAINED for parity / off-chain consumers. V3 escrows use the ATTESTED fee at
     *      release, which may differ from this value.
