...
def kernel_signing_proof_message(kernel_id):
    ...
    (``packages/kernel/src/kernel-keychain.ts``). Do NOT add an env tag or nonce
    here: the binding wire contract pins this exact string for v1;
...
def register_signing_key(pcc_base, api_key, kernel_id, public_key_hex, secret_key_hex):
    ...
    ``signingKey:{algorithm:"ed25519", publicKey}`` on the kernel row -- the
    registry value the oracle's ``machine.execution_log`` verifier resolves to
    check machine-log signatures. This is the key that ACTUALLY signs the node's
    machine-log hash chain (``log_capture``), so it is the key registration must
    bind (set-once): registering any other identity would bind a signer that
    never signs the logs -> settlement mismatch forever.

    Endpoint: ``POST /api/kernels`` (the registration/upsert route -- the gateway
    verifies the proof and persists the signing key on the existing kernel row,
    keeping ONE proof route). The kernel already exists (``register_kernel`` runs
    first), so this is the upsert path. Body (field names are the binding
    contract):
