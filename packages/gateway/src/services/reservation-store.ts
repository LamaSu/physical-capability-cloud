 ...
 *   2. `acceptedDealDigest` is RECOMPUTED from the plan's content and must equal the carried digest
 *      (the #351 consumer contract). A carried digest proves nothing on its own.
 ...
 */
import { acceptedDealDigest, type Address } from "@pcc/spec";
...
export function reservationWiring(store: BudgetReservationStore): ReservationWiring {
  return {
    ...
    consumeReservation(reservationId, principal, plan, now) {
      ...
      if (acceptedDealDigest(rest) !== carried.toLowerCase()) return { ok: false, reason: "digest-mismatch" };
      ...
      const r = store.consume({
        ...
        dealDigest: carried.toLowerCase(),
        now,
      });
